...
from cube_stacking.utils import CAM_POSES, Rewards, RandomPositions, Player, WEIGHT_COLORS

MAX_HEIGHT = 10
TEST_STEPS = 50
COLOR_PREVIEW_SIZE = 8
...
class TwoPlayerDirectoryFullWeightedArena(gym.Env):
    ...
    def fall_higher_obs(self, action, player):
        ...
        higher = self.sim.place_cube(action, color, weight=self.weights[self.next_weight])
        fall = self.sim.last_cube_fell(TEST_STEPS)
        obs = self.add_preview(self.sim.render())
    ...
    def step(self, action):
        ...
        if (higher and fall_opponent) or not higher:
    ...
    def add_preview(self, obs):
        self.next_weight = np.random.randint(0, len(self.weights))
        color = WEIGHT_COLORS[self.weights[self.next_weight]][:3]
        color_segment = np.tile(color,(COLOR_PREVIEW_SIZE,COLOR_PREVIEW_SIZE,1)) * 255
        obs[:COLOR_PREVIEW_SIZE, :COLOR_PREVIEW_SIZE, :] = color_segment.astype(np.uint8)
        return obs
...
if __name__ == '__main__':
    ...
    env = gym.make("Cubestacc-TwoPlayer-Full-Weighted-NonRandom-PPO-Graphical-v2")
